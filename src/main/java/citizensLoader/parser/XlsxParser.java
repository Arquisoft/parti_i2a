...
public class XlsxParser implements Parser {
	...
	/**
	 * Reads an excel file and inserts in the Citizen database data read from
	 * it.
	 */
	@Override
	...
	/**
	 * Adds new citizens (read from the excel file) to the specified list.
	 * 
	 * @param users
	 *            - citizens list
	 * @param sheet
	...
	private void parseUsers(List<User> users, XSSFSheet sheet, int rows) throws IOException {
		...
		for (int r = 1; r < rows; r++) {
			...
			if (row != null) {
				...
				User citizen = new User(dni, nombre, apellidos, nacimiento, direccion, email, nacionalidad, polling);
